...
use Clue\React\Redis\LazyStreamingClient;
...
class LazyStreamingClientTest extends TestCase
{
    private $factory;
    private $stream;
    private $parser;
    ...
    {
        $this->factory = $this->getMockBuilder('Clue\React\Redis\Factory')->setConstructorArgs(array(Factory::create()))->getMock();
        ...
        $this->factory->expects($this->any())->method('createClient')->with('localhost')->will($this->returnValue(new FulfilledPromise(new StreamingClient($this->stream, $this->parser, $this->serializer))));

        $this->client = new LazyStreamingClient('localhost', $this->factory);
    }
    ...
    {
        $client = new LazyStreamingClient('localhost', $this->factory);
    }
